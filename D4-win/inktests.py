...
os.makedirs("utdiffs", exist_ok=True)
...
def run_screenshot(filename: str):
    b, _ = os.path.splitext(filename)
    subprocess.call([SCUMMVM_PATH, f"--start-movie={filename}", "--debugflags=fewframesonly,fast", "director"])

    ax = subprocess.run(["compare", "-metric", "NCC", "-fuzz", "10%", f"baseline/{b}.png", f"utscreen/{b}.png", f"utdiffs/{b}.png"], capture_output=True)



files = [f for f in os.listdir(".") if f.startswith("INK") and f.endswith(".DIR")]
with multiprocessing.Pool(9) as pool:
    pool.map(run_screenshot, files)
...
def run_compare(basename: str, width: int, height: int, xpos: int, ypos: int) -> bool:
    args = ["compare", "-metric", "AE", "-fuzz", "10%", "-crop", f"{width}x{height}+{xpos}+{ypos}", f"baseline/{basename}.png", f"utscreen/{basename}.png", f"NULL:"]
    ax = subprocess.run(args, capture_output=True)
    return ax.stderr.isdigit() and int(ax.stderr) == 0
